...
def test_blended_probs_uses_live_market_and_includes_market_only_teams():
    ...
    assert "Croatia" not in OPTA                    # guard: keep this example genuinely market-only
    live = {"Spain": 0.16, "Croatia": 0.03}        # Spain is in Opta; Croatia is market-only
    b = blended_probs(market=live)
    assert b["Spain"] == pytest.approx(0.5 * OPTA["Spain"] + 0.5 * 0.16)
    assert b["Croatia"] == pytest.approx(0.03)      # market-only team included at its market prob
